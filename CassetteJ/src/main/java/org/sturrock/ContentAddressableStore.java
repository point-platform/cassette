...
public interface ContentAddressableStore {
	 ...
	 * If the store already contains this content, the write is discarded but
	 * the hash is returned as normal.
	 * @param stream	Content to be written
	 ...
	 * @param hash	Hash of content to check
	 * @return		<code>true</code> if content exists in the store with
	 * 				specified hash
	 */
	 ...
	 * @param hash	The hash of the content to read.
	 ...
	 * @return		The length of the content, or -1 if no content with the specified hash exists.
	 * @throws IOException
	 */
	long getContentLength(byte[] hash) throws IOException;

	/**
	 * Get a list of all hashes in the store.  The list is generated lazily
	 * by querying the filesystem, and thus will not behave deterministically if
	 ...
	 * @param	hash Hash of content to delete
	 * @return	<code>true</code> if the content existed and was deleted; otherwise false.
	 * @throws IOException
	 */
